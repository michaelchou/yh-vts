...
if(strClientIP == null || strClientIP.length() == 0 || "unknown".equalsIgnoreCase(strClientIP))
 ...
 System.out.println("strClientMAC:" + strClientMAC + "   " + "TerminalMV.jsp" );
   //根据当前终端MAC地址查询其详细信息
   TerminalDB terminaldb = new TerminalDB();
   Terminal terminal = terminaldb.getTerminalByMACAddr(strClientMAC);
...
function showTakeCardTip4Err()
{
  ...
  top.serviceCtrl.startFlowCtrlTimeout(function(){top.serviceCtrl.CommErrAfter2IdleMV();}, 30*1000);
}
...
function onServerErr_Def(frmname, err)
{
  ...
  if (top.cardreader.isCardPresent())
  {
    onDeviceError_Idc = onCardCaptured = onCardTaken = function()
    {
      ...
      top.serviceCtrl.startFlowCtrlTimeout(function(){top.serviceCtrl.CommErrAfter2IdleMV();}, 60*1000);
    }
  ...
  else
  {
    ...
    top.serviceCtrl.startFlowCtrlTimeout(function(){top.serviceCtrl.setSvcStatus(top.TTSTATUS_COMMERR); top.serviceCtrl.CommErrAfter2IdleMV();}, 10*1000);
    top.wins.showInfoTip("对不起，本机通讯中断，暂停服务！<br/><br/>Sorry, communication error!");
  }
}
</script>
...
function onDocumentCompleted()
{
  ...
  {
	 wins.showInfoTip("终端初始化失败（终端无配置读卡器）！");
     setTimeout(function(){top.location.reload(true);}, 5*60*1000);
     return;
  } 
  else{
  ...
  {
    wins.showInfoTip("终端初始化失败（终端无配置密码键盘）！");
    setTimeout(function(){top.location.reload(true);}, 5*60*1000);
    return;
  }
  else{
...
function startSvc()
{
  ...
  terminal.devType = devType;
  terminal.isSmartTeller = false;//移动智能柜员机
  pinpad = new Pinpad();
  journalPrinter = new JournalPrinter();
  try{guidelights = new GuideLights();}catch(e){}
  try{if(typeof(YHAXGuideLights) != "undefined"){guidelights.setPinPadLight("CONTINUOUS");}}catch(e){} //密码键盘灯
...
function resetCardReader()
{
  onResetComplete_Idc = onDeviceError_Idc = function()
  {
	...
	top.journalPrinter.addJournal(strJrn);
    // 跳至服务空闲
    setTimeout(function(){top.serviceCtrl.navigate2IdleMV();}, 500);
  ...
  setInterval(function(){
	try{
	top.journalPrinter.addJournalWithTime("* * * * * 心跳检测");
      top.YHAXCommonCtrl.CanConnectToServer(top.serviceCtrl.strServerIP, top.serviceCtrl.iServerPort);
    } catch(e){}
 },50*1000);
...
<div id="oLInfoTip" class="FULLSCR">
  <table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
    <tr><td align="center">
	  <br/>
      <span class="Tip_Title" ID="InfoTipContent">移动终端正在初始化,请稍候......</span><br/><br/>
    </td></tr>
  </table>
</div>
